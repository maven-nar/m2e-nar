...
public final class MavenUtils {
	...
	private static <T> T loadMojo(final IMaven maven,
			final MavenProject project, final MojoExecution mojoExecution,
			final Class<T> asType, final IProgressMonitor monitor)
			throws CoreException {
		return maven.createExecutionContext().execute(project,
				new ICallable<T>() {
					public T call(IMavenExecutionContext context,
							IProgressMonitor monitor) throws CoreException {
						return maven.getConfiguredMojo(context.getSession(),
								mojoExecution, asType);
					}
				}, monitor);
	}

	private static void releaseMojo(final IMaven maven,
			final MavenProject project, final Object mojo,
			final MojoExecution mojoExecution, final IProgressMonitor monitor)
			throws CoreException {
		maven.createExecutionContext().execute(project,
				new ICallable<Boolean>() {
					public Boolean call(IMavenExecutionContext context,
							IProgressMonitor monitor) throws CoreException {
	...
	private static synchronized ClassRealm getMyRealm(ClassWorld world)
			throws CoreException {
		try {
			if (realm == null) {
				realm = world.newRealm(MavenNarPlugin.PLUGIN_ID,
						CProjectConfigurator.class.getClassLoader());
			}
			return realm;
		} catch (DuplicateRealmException e) {
			throw new CoreException(new Status(IStatus.ERROR,
					MavenNarPlugin.PLUGIN_ID,
					"Problem when creating realm", e));
		}
	}

	private static <T extends AbstractMojo> T getConfiguredMojo(final IMaven maven,
			final MavenProject project, final MojoExecution mojoExecution,
		...
		return maven.createExecutionContext().execute(project,
				new ICallable<T>() {
					public T call(IMavenExecutionContext context,
							IProgressMonitor monitor) throws CoreException {
						return getConfiguredMojo(context.getSession(),
								mojoExecution, asType, log);
					}
	...
	private static <T extends AbstractMojo> T getConfiguredMojo(MavenSession session,
		...
		ClassRealm pluginRealm = getMyRealm(pluginDescriptor.getClassRealm()
				.getWorld());
	...
	private static void populatePluginFields(Object mojo,
			MojoDescriptor mojoDescriptor, ClassRealm pluginRealm,
		...
		try {
			...
			configurator.configureComponent(mojo, configuration,
					expressionEvaluator, pluginRealm);
			...
			throw new CoreException(new Status(IStatus.ERROR,
					MavenNarPlugin.PLUGIN_ID, message, e));
		} catch (NoClassDefFoundError e) {
			ByteArrayOutputStream os = new ByteArrayOutputStream(1024);
			PrintStream ps = new PrintStream(os);
			ps.println("A required class was missing during configuration of mojo "
					+ mojoDescriptor.getId() + ": " + e.getMessage());
			pluginRealm.display(ps);

			throw new CoreException(new Status(IStatus.ERROR,
					MavenNarPlugin.PLUGIN_ID, os.toString(), e));
	...
	public static NarExecution readCompileSettings(
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final MojoExecution compileExecution,
			final IProgressMonitor monitor)
			throws CoreException {
		return readSettings(context, facade, compileExecution, NarCompileMojo.class, NarExecution.MAIN, monitor);
	}
	
	public static NarExecution readTestCompileSettings(
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final MojoExecution compileExecution,
			final IProgressMonitor monitor)
	...
	public static <T extends AbstractCompileMojo> NarExecution readSettings(
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final MojoExecution compileExecution,
			final Class<T> mojoType,
		...
		if (compileExecution != null) {
			...
			AbstractMojo narMojo = loadMojo(maven, mavenProject,
					compileExecution, AbstractMojo.class, monitor);
			try {
				...
				T compileMojo = getConfiguredMojo(maven,
						mavenProject, compileExecution, mojoType, narMojo.getLog(), monitor);
				compileMojo.setNarProperties(new NarProperties(mavenProject,
						narMojo.getClass()));
				...
				NarExecutionBuilder builder = new NarExecutionBuilder(
						compileMojo, compileExecution);
				settings = builder.build(buildType);
			} catch (MojoFailureException e) {
				throw new CoreException(new Status(IStatus.ERROR,
						MavenNarPlugin.PLUGIN_ID,
						"Couldn't configure mojo"));
			} catch (MojoExecutionException e) {
				throw new CoreException(new Status(IStatus.ERROR,
						MavenNarPlugin.PLUGIN_ID,
						"Couldn't configure mojo"));
			} finally {
				releaseMojo(maven, mavenProject, narMojo, compileExecution,
						monitor);
			}
		} else {
			throw new CoreException(new Status(IStatus.ERROR,
	...
	public static List<MojoExecution> getCompileExecutions(
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final IProgressMonitor monitor)
			throws CoreException {
	...
	public static List<MojoExecution> getTestCompileExecutions(
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final IProgressMonitor monitor)
			throws CoreException {
	...
	public static List<MojoExecution> getExecutions(
			final String goal,
			final ConfiguratorContext context,
			final IMavenProjectFacade facade,
			final IProgressMonitor monitor)
	...
	public static List<NarExecution> buildCompileNarExecutions(final ConfiguratorContext context, final IMavenProjectFacade facade, final IProgressMonitor monitor) throws CoreException {
		...
		List<MojoExecution> compileExecutions = MavenUtils
				.getCompileExecutions(context, facade, monitor);
		for (MojoExecution compileExecution : compileExecutions) {
			NarExecution narSettings = MavenUtils.readCompileSettings(context,
					facade, compileExecution, monitor);
	...
	public static List<NarExecution> buildTestCompileNarExecutions(final ConfiguratorContext context, final IMavenProjectFacade facade, final IProgressMonitor monitor) throws CoreException {
		List<NarExecution> narExecutions = new ArrayList<NarExecution>();
		List<MojoExecution> testCompileExecutions = MavenUtils
				.getTestCompileExecutions(context, facade, monitor);
		for (MojoExecution testCompileExecution : testCompileExecutions) {
			NarExecution narSettings = MavenUtils.readTestCompileSettings(context,
					facade, testCompileExecution, monitor);
			narExecutions.add(narSettings);
		}
		return narExecutions;
	}

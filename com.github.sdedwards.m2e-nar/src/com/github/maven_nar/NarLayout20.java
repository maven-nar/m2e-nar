...
public class NarLayout20 extends AbstractNarLayout {
	 ...
	 */
	public File getNoArchDirectory(File baseDir, NarArtifact artifact) throws MojoExecutionException, MojoFailureException {
		return baseDir;
	}

	public File getNoArchDirectory(File baseDir, MavenProject project) throws MojoExecutionException, MojoFailureException {
		return baseDir;
	}
	 ...
	 */
	public final File getLibDirectory(File baseDir, NarArtifact artifact, String aol, String type) throws MojoFailureException {
	 ...
	 * com.github.maven_nar.AOL, String type)
	 */
	public final File getLibDirectory(File baseDir, MavenProject project, String aol, String type) throws MojoFailureException {
		if (type.equals(Library.EXECUTABLE)) {
			throw new MojoFailureException("INTERNAL ERROR, Replace call to getLibDirectory with getBinDirectory");
		}
	 ...
	 * if ( getIncludeDirectory( baseDir, project ).exists() ) { narInfo.setNar(
	 * null, "noarch", project.getGroupId() + ":" + project.getArtifactId() +
	 * ":" + NarConstants.NAR_TYPE + ":" + "noarch" ); }
	 * 
	 * String[] binAOL = new File( baseDir, "bin" ).list(); for ( int i = 0; (
	 * binAOL != null ) && ( i < binAOL.length ); i++ ) {// TODO: chose not to
	 * apply new file naming for outfile in case of backwards compatability, may
	 * need to reconsider narInfo.setNar( null, Library.EXECUTABLE,
	 ...
	 * 
	 * File libDir = new File( baseDir, "lib" ); String[] libAOL =
	 * libDir.list(); for ( int i = 0; ( libAOL != null ) && ( i < libAOL.length
	 * ); i++ ) { String bindingType = null; String[] libType = new File(
	 * libDir, libAOL[i] ).list(); for ( int j = 0; ( libType != null ) && ( j <
	 * libType.length ); j++ ) { narInfo.setNar( null, libType[j],
	 * project.getGroupId() + ":" + project.getArtifactId() + ":" +
	 * NarConstants.NAR_TYPE + ":" + "${aol}" + "-" + libType[j] );
	 * 
	 * // set if not set or override if SHARED if ( ( bindingType == null ) ||
	 * libType[j].equals( Library.SHARED ) ) { bindingType = libType[j]; } }
	 * 
	 * AOL aol = new AOL( libAOL[i] ); if ( narInfo.getBinding( aol, null ) ==
	 * null ) { narInfo.setBinding( aol, bindingType != null ? bindingType :
	 * Library.NONE ); } if ( narInfo.getBinding( null, null ) == null ) {
	 * narInfo.setBinding( null, bindingType != null ? bindingType :
	 * Library.NONE ); } } }
	 * 
	 * public File getNarUnpackDirectory(File baseUnpackDirectory,
	 * AttachedNarArtifact artifact ) { return baseUnpackDirectory; }
	 */
}
